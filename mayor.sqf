...
if (!ILG_Voting_Enable) exitWith {player groupchat "The dictator has stopped all elections. Your vote will not count.";};
...
if (_art == "CollectTaxes") then
{
	if (mayorPayAtGov < 10000) exitWith {player groupchat format ["There isn't enough tax payers money in the vault, theres only $%1",mayorPayAtGov];};
	["Mishy",mayorPayAtGov] call INV_addinvitem;
	Format ['server globalchat "The governer has just collected the taxes from the voting booth!";'] call broadcast;
	Format ['server globalchat format ["He collected $%1 of the tax payers money!",%1];',mayorPayAtGov] call broadcast;
	mayorPayAtGov = 0;
};