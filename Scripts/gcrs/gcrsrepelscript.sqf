...
if (_vehclass == "Mi17_Civilian") then 
	{
	player setpos [(getpos vehicle player select 0) - 1.25, (getpos vehicle player select 1) + 5.5, (getpos vehicle player select 2) - 4];
	};
...
if (_vehclass == "Mi17_Civilian") then 
	{
	player setposasl [_xpos - 1.25, _ypos + 5.5, _endingheight];
	};
