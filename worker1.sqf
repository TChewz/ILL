...
while {true} do 
	...
	for [{_i=0}, {_i < 120}, {_i=_i+1}] do 
		...
		if ( (not(alive _hurenname)) or (not(alive player)))  exitWith {sleep 2; if(!isnull _hurenname)then{player groupChat localize "STRS_huren_tot"}else{player groupChat "A hooker you owned has been sent home! you are now wanted for being a poor pimp."}; localhuren = localhuren - 1; _hurenname setDammage 1; _exitvar = 1;};
