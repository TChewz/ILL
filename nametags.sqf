...
for [{_i=1}, {_i < 25}, {_i=_i+2}] do
	...
	if(count _objs > 0 and INV_shortcuts)then
		...
		if(_tag in shopusearray and _distTP < 3) then
			...
			titleRsc["Rtags", "PLAIN"];
			_control = (uiNamespace getVariable 'TAGS_HUD') displayCtrl 64438;
			if(iscop and _tag in drugsellarray)exitwith{_control ctrlSetText "Illegal Items Search (E)";_control ctrlSetTextColor [0, 0, 1, 0.8]};
		...
		if (_tag != player and _tag in playerarray and alive _tag) then
			...
			{
				if (_tag in [Cop19,Cop20]) then
				{
					switch (_tag) do
					{
						case Cop19:{_control2 ctrlSetText format ["%1 (%2)", "Civ69",(name _tag)];};
						case Cop20:{_control2 ctrlSetText format ["%1 (%2)", "Civ70",(name _tag)];};
					};
			...
			_esud = false;
			if (_tag in [Civ51,Civ52,Civ53,Civ54,Civ55]) then
			{
			...
			if(_tag in civarray && !_esud)then

				{
					...
					if (!_colorset) then
					{
						_control2 ctrlSetTextColor [1, 1, 1, 0.8];//Default one
					};
				};
