...
INV_GetStorageWeight = {
	...
	for [{_c=0}, {_c < (count _Array)}, {_c=_c+1}] do {
		if (!isNil "INV_getitemTypeKg") then {
...
INV_getshopnum = {
	...
	_Fshopnum = [];
	if ((typeName _this) == "OBJECT") then {
		diag_log "Passed Object Check";
		for [{_c=0}, {_c < (count INV_itemshops)}, {_c=_c+1}] do {
			if (((INV_itemshops select _c) select 0) == _this) then {
				_Fshopnum = _c;
				diag_log format ["Passed Object Verification: %1", _fshopnum];
			};
		};
	};
...
for "_c" from 0 to (count gangsarray - 1) do
	...
	_gangarray1 = gangsarray select _c;
	_gang1 = _gangarray1 select 0;
	_members1 = _gangarray1 select 1;
	if(name _this in _members1)then{_mygang1 = _gang1;};
...
INV_HasIllegalItems = {
	_Ret = false;
	{
		...
		if (_Item call INV_getitemIsIllegal && (_NumberOfIllegalItems != 0)) exitWith {
			_Ret = true;
		};
