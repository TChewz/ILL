...
_handled = false;
/*
if(isstunned || (animationState player) == ILG_Restrain_Anim || player call ISSE_IsVictim) exitwith

	{

	if(_key == 50)then{_handled = true};
	if(_key == 11)then{_handled = true};
	if(_key == 21) then {_handled = true};
	_handled
...

{
...

	{
		...
		{
			INV_shortcuts=false;
			titletext["@ILL keys off", "PLAIN DOWN"];
			[] execVM "actions\actionsRemove.sqs"
		}else{
			INV_shortcuts=true;
			titletext["@ILL keys on", "PLAIN DOWN"];
	...
	if (!(_vcl call Own_This_Vehicle)) exitWith {player groupchat "You don't have the keys to this vehicle!"; _handled = true;};
	...
	for [{_i=1}, {_i < 3}, {_i=_i+1}] do
		...
		if(vehicle player != player ||  (player getVariable "restrained")) exitwith{};
	...
	case 6:
	{
		if(!INV_shortcuts)exitwith{};
		if (dialog) exitWith {closeDialog 0};
		execVM "Juan\Civs\Gangs\mainFunction.sqf";
		_handled = true;
	};
	...
	case 35:
	{
		...
		if(!INV_shortcuts)exitwith{};
		if (vehicle player != player) exitWith {player groupchat "You cannot holster/unholster a weapon whilst in a vehicle.";};
