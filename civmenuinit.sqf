...
if(!(_civmenuciv call ISSE_IsVictim))exitwith{hint localize "STRS_inventory_checknohands"};
if(_civmenuciv getVariable ["RestrainedBy", -1] == 0) exitWith{};
(format ["if (player == %1) then {[""stealmoney"", ""%2""] execVM ""civmenu.sqf""};", _civmenu_civ, player]) call broadcast;
...
if ((_art == 3) and (player distance prisonflag <= 75)) exitWith
...
{
_check = call (compile format ["%1_warning", _civmenuciv]);
if (count _check == 0) exitwith {hint "Not Warned";};
