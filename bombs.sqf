...
//				if (player distance _this < 25) then {player setDammage 1;}; // All players within 25m die instantly
				};
...
if (_art == "config") then
{
	...
	switch _name do
	{
		case "fernzuenderbombe":      {if (_art == "defuse") then {format["""%1"" call INV_BombDelete", _id] call broadcast;[_name, 1] call INV_addinvitem;player groupChat localize "STRS_inv_item_vehiclebomb_defused";};if (_art == "activate") then {format["if (isServer) then {[""%1"", [%2]] call INV_BombEdit;};", _id, true] call broadcast;};};
		case "zeitzuenderbombe":      {if (_art == "defuse") then {format["""%1"" call INV_BombDelete", _id] call broadcast;[_name, 1] call INV_addinvitem;player groupChat localize "STRS_inv_item_vehiclebomb_defused";};if (_art == "timer") exitWith {if (!(createDialog "timebombconfig")) exitWith {hint "Dialog Error!";};_sliderpos = 0;ctrlSetText [4, localize "STRS_inv_item_vehiclebomb_changetimer"];sliderSetRange [1, zeitbombe_mintime, zeitbombe_maxtime];sliderSetSpeed [1, 1, 10];buttonSetAction [4, "zeitbombencounterchanged = 1; closedialog 0;"];zeitbombencounterchanged = 0;while {ctrlVisible 2} do {_sliderpos = round(sliderPosition 1);ctrlSetText [3, format[localize "STRS_inv_item_vehiclebomb_sec", _sliderpos]];sleep 0.3;};if (zeitbombencounterchanged == 1) then {format["if (isServer) then {[""%1"", [%2]] call INV_BombEdit;};", _id, (time+_sliderpos)] call broadcast;player groupChat format [localize "STRS_inv_item_vehiclebomb_changed", _sliderpos];};};};
		case "aktivierungsbombe":     {if (_art == "defuse") then {format["""%1"" call INV_BombDelete", _id] call broadcast;[_name, 1] call INV_addinvitem;player groupChat localize "STRS_inv_item_vehiclebomb_defused";};};
		case "geschwindigkeitsbombe": {if (_art == "defuse") then {format["""%1"" call INV_BombDelete", _id] call broadcast;[_name, 1] call INV_addinvitem;player groupChat localize "STRS_inv_item_vehiclebomb_defused";};};
	};
};
